...
contract(
  ...
  ([operator, admin, owner, bullUser1, bullUser2, bullUser3, bearUser1, bearUser2, bearUser3]) => {
    ...
    beforeEach(async () => {
      ...
      mockUsdc.approve(prediction.address, ApproveAmount, { from: bullUser1 });
      mockUsdc.approve(prediction.address, ApproveAmount, { from: bullUser2 });
      mockUsdc.approve(prediction.address, ApproveAmount, { from: bullUser3 });
      mockUsdc.approve(prediction.address, ApproveAmount, { from: bearUser1 });
      mockUsdc.approve(prediction.address, ApproveAmount, { from: bearUser2 });
      mockUsdc.approve(prediction.address, ApproveAmount, { from: bearUser3 });
    });

    it("Initialize", async () => {
      assert.equal(await mockUsdc.balanceOf(prediction.address), 0);
      ...
      assert.equal(await prediction.minParticipateAmount(), MIN_BET_AMOUNT.toString());
    ...
    it("Should record data and user bets", async () => {
      ...
      await prediction.participateLong(currentEpoch, ether("1.1"), { from: bullUser1 }); // 1.1 CAKE
      await prediction.participateLong(currentEpoch, ether("1.2"), { from: bullUser2 }); // 1.2 CAKE
      await prediction.participateShort(currentEpoch, ether("1.4"), { from: bearUser1 }); // 1.4 CAKE

      assert.equal((await mockUsdc.balanceOf(prediction.address)).toString(), ether("3.7").toString()); // 3.7 CAKE
      ...
      await prediction.participateLong(currentEpoch, ether("2.1"), { from: bullUser1 }); // 2.1 CAKE
      await prediction.participateLong(currentEpoch, ether("2.2"), { from: bullUser2 }); // 2.2 CAKE
      await prediction.participateShort(currentEpoch, ether("2.4"), { from: bearUser1 }); // 2.4 CAKE

      assert.equal((await mockUsdc.balanceOf(prediction.address)).toString(), ether("10.4").toString()); // 10.4 CAKE (3.7+6.7)
      ...
      await prediction.participateLong(currentEpoch, ether("3.1"), { from: bullUser1 }); // 3.1 CAKE
      await prediction.participateLong(currentEpoch, ether("3.2"), { from: bullUser2 }); // 3.2 CAKE
      await prediction.participateShort(currentEpoch, ether("3.4"), { from: bearUser1 }); // 3.4 CAKE

      assert.equal((await mockUsdc.balanceOf(prediction.address)).toString(), ether("20.1").toString()); // 20.1 CAKE (3.7+6.7+9.7)
      ...
      await prediction.participateLong(currentEpoch, ether("4.1"), { from: bullUser1 }); // 4.1 CAKE
      await prediction.participateLong(currentEpoch, ether("4.2"), { from: bullUser2 }); // 4.2 CAKE
      await prediction.participateShort(currentEpoch, ether("4.4"), { from: bearUser1 }); // 4.4 CAKE

      assert.equal((await mockUsdc.balanceOf(prediction.address)).toString(), ether("32.8").toString()); // 32.8 CAKE (3.7+6.7+9.7+12.7)
    ...
    it("Should not allow multiple bets", async () => {
      ...
      await prediction.participateLong(currentEpoch, ether("1"), { from: bullUser1 }); // Success
      await expectRevert(
        prediction.participateLong(currentEpoch, ether("1"), { from: bullUser1 }),
        "Can only bet once per round"
      );
      await expectRevert(
        prediction.participateShort(currentEpoch, ether("1"), { from: bullUser1 }),
        "Can only bet once per round"
      );
      await prediction.participateShort(currentEpoch, ether("1"), { from: bearUser1 }); // Success
      await expectRevert(
        prediction.participateLong(currentEpoch, ether("1"), { from: bearUser1 }),
        "Can only bet once per round"
      );
      await expectRevert(
        prediction.participateShort(currentEpoch, ether("1"), { from: bearUser1 }),
        "Can only bet once per round"
      );
    ...
    it("Should not allow bets lesser than minimum bet amount", async () => {
      ...
      await expectRevert(
        prediction.participateLong(currentEpoch, ether("0.5"), { from: bullUser1 }),
        "Bet amount must be greater than minParticipateAmount"
      ); // 0.5 CAKE
      await prediction.participateLong(currentEpoch, ether("1"), { from: bullUser1 }); // Success
    ...
    it("Should record rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockUsdc.balanceOf(prediction.address)).toString(), ether("3.7").toString());
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal(
        (await mockUsdc.balanceOf(prediction.address)).toString(),
    ...
    it("Should claim rewards", async () => {
      ...
      await prediction.participateLong(currentEpoch, ether("1"), { from: bullUser1 }); // 1 CAKE
      await prediction.participateLong(currentEpoch, ether("2"), { from: bullUser2 }); // 2 CAKE
      await prediction.participateShort(currentEpoch, ether("4"), { from: bearUser1 }); // 4 CAKE
      ...
      await prediction.participateLong(currentEpoch, ether("21"), { from: bullUser1 }); // 21 CAKE
      await prediction.participateLong(currentEpoch, ether("22"), { from: bullUser2 }); // 22 CAKE
      await prediction.participateShort(currentEpoch, ether("24"), { from: bearUser1 }); // 24 CAKE
    ...
    it("Should claim treasury rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockUsdc.balanceOf(prediction.address)).toString(), predictionCurrentCAKE.toString());
      ...
      await prediction.participateLong(currentEpoch, ether("31"), { from: bullUser1 }); // 31 CAKE
      await prediction.participateLong(currentEpoch, ether("32"), { from: bullUser2 }); // 32 CAKE
      await prediction.participateShort(currentEpoch, ether("34"), { from: bearUser1 }); // 34 CAKE
    ...
    it("Admin/Owner function work as expected", async () => {
      ...
      tx = await prediction.setMinParticipateAmount("50", { from: admin });
      expectEvent(tx, "NewMinParticipateAmount", { minParticipateAmount: "50" });
      await expectRevert(prediction.setMinParticipateAmount("0", { from: admin }), "Must be superior to 0");
    ...
    it("Should reject admin/owner functions when not admin/owner", async () => {
      ...
      await expectRevert(prediction.setMinParticipateAmount("0", { from: bullUser1 }), "Not admin");
    ...
    it("Should reject admin/owner functions when not paused", async () => {
      ...
      await expectRevert(prediction.setMinParticipateAmount("0", { from: admin }), "Pausable: not paused");
    ...
    it("Should refund rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockUsdc.balanceOf(prediction.address)).toString(), ether("7").toString());
    ...
    it("Rejections for bet bulls/bears work as expected", async () => {
      // Epoch 0
      await expectRevert(prediction.participateLong("0", ether("1"), { from: bullUser1 }), "Round not bettable");
      await expectRevert(prediction.participateShort("0", ether("1"), { from: bullUser1 }), "Round not bettable");
      await expectRevert(prediction.participateLong("1", ether("1"), { from: bullUser1 }), "Bet is too early/late");
      await expectRevert(prediction.participateShort("1", ether("1"), { from: bullUser1 }), "Bet is too early/late");
      ...
      await prediction.genesisStartRound();
      currentEpoch = await prediction.currentEpoch();
      await expectRevert(prediction.participateLong("1", ether("101"), { from: bullUser1 }), "transfer amount exceeds balance");
      await expectRevert(prediction.participateShort("1", ether("101"), { from: bearUser1 }), "transfer amount exceeds balance");
      await expectRevert(prediction.participateLong("2", ether("1"), { from: bullUser1 }), "Bet is too early/late");
      await expectRevert(prediction.participateShort("2", ether("1"), { from: bullUser1 }), "Bet is too early/late");

      // Bets must be higher (or equal) than minParticipateAmount
      await expectRevert(
        prediction.participateShort("1", ether("0.999999"), { from: bullUser1 }),
        "Bet amount must be greater than minParticipateAmount"
      );
      await expectRevert(
        prediction.participateLong("1", ether("0.999999"), { from: bullUser1 }),
        "Bet amount must be greater than minParticipateAmount"
      );
    });
    ...
    it("Should prevent betting when paused", async () => {
      ...
      await expectRevert(prediction.participateLong(currentEpoch, ether("1"), { from: bullUser1 }), "Pausable: paused");
      await expectRevert(prediction.participateShort(currentEpoch, ether("1"), { from: bearUser1 }), "Pausable: paused");
    ...
    it("Should paginate user rounds", async () => {
      ...
      await prediction.participateLong(currentEpoch, ether("1"), { from: bullUser1 });
      await prediction.participateLong(currentEpoch, ether("1"), { from: bullUser2 });
      await prediction.participateShort(currentEpoch, ether("1"), { from: bearUser1 });
    ...
    it("recoverToken function work as expected", async () => {
      ...
      await prediction.participateLong(currentEpoch, ether("1"), { from: bullUser2 });
      await prediction.participateShort(currentEpoch, ether("1"), { from: bearUser1 });
      assert.equal(await mockUsdc.balanceOf(prediction.address), ether("3").toString());
      await expectRevert(
        prediction.recoverToken(mockUsdc.address, ether("1"), { from: owner }),
      ...
      const tx = await prediction.recoverToken(randomToken.address, ether("66"), { from: owner });
